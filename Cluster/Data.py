...
def process_data(cluster_number,cluster_data_path, feature_data_path, response_dir, output_dir):
    ...
    for cluster in cluster_df.Cluster.unique():
        if int(cluster) == cluster_number:
            ...
            pretrain_df, drugs = load_cluster_data(cluster_df, features, cluster, response_dir)
            
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            
            new_dir = os.path.join(output_dir, f'cluster_{cluster}_responses')
