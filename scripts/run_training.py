...
from refract.metrics import get_stringdb_network_interactions, get_merged_shap_values_and_features, get_top_k_features, get_test_predictions
...
def run(response_path, feature_path, output_dir):
    ...
    for idx, (train_val_index, test_index) in enumerate(outer_splitter.split(response_df)):
        logger.info("Training fold: {}".format(idx))
        train_val = response_df.iloc[train_val_index, :].copy()
        train_index, val_index = next(KFold(n_splits=4, shuffle=True, random_state=42).split(train_val))
