...
def get_kendall_tau_correlations(response_df, feature_df, executor=None):
    ...
    def compute_kendall(idx):
        ...
        return {"feature_name": feature_name, "correlation": corr, "p_value": p}

    if executor is None:
        with ThreadPoolExecutor() as executor:
            results = list(
                tqdm(executor.map(compute_kendall, range(n_features)), total=n_features)
            )
    else:
        results = list(
            tqdm(executor.map(compute_kendall, range(n_features)), total=n_features)
...
def get_pearson_correlations(
    response_df: pd.DataFrame | pd.Series,
    ...
    *,
    ...
    out = pd.DataFrame(
        {"feature_name": X.columns, "correlation": r.values, "p_value": p, "n": n}
    )
...
def get_top_n_features(feat_corr_df, n):
    ...
    feat_corr_df = feat_corr_df[feat_corr_df.p_value < 0.05]
