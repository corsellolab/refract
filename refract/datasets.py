...
class FeatureSet:
    ...
    def load_feature_tables(self):
        ...
        """
        self.feature_tables["GE"] = pd.read_csv(f"{self.feature_dir}/ge.csv", index_col=0)
        self.feature_tables["CNA"] = pd.read_csv(f"{self.feature_dir}/cna.csv", index_col=0)
        self.feature_tables["MET"] = pd.read_csv(f"{self.feature_dir}/met.csv", index_col=0)
        self.feature_tables["miRNA"] = pd.read_csv(f"{self.feature_dir}/mirna.csv", index_col=0)
        self.feature_tables["MUT"] = pd.read_csv(f"{self.feature_dir}/mut.csv", index_col=0)
        self.feature_tables["PROT"] = pd.read_csv(f"{self.feature_dir}/prot.csv", index_col=0)
        self.feature_tables["XPR"] = pd.read_csv(f"{self.feature_dir}/xpr.csv", index_col=0)
        self.feature_tables["LIN"] = pd.read_csv(f"{self.feature_dir}/lin.csv", index_col=0)
        self.feature_tables["shRNA"] = pd.read_csv(f"{self.feature_dir}/shrna.csv", index_col=0)
        self.feature_tables["repurposing"] = pd.read_csv(f"{self.feature_dir}/rep.csv", index_col=0)
...
class ResponseSet:
    ...
    def get_joined_features(self, pert_name: str, pert_mfc_id:str, \
                            dose: float, feature_set: FeatureSet, feature_name:str) -> Tuple[pd.DataFrame, pd.DataFrame]:
