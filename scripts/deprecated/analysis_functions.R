...
random_forest <- function(X_train, X_test, Y_train, Y_test, n = 500){  
  yhat_rf <- Y_test
    ...
    X_train <- X_train[,rank(-abs(cor(X_train, Y_train)))<= n]
    
    # makes RF and ENet models
    rf <- ranger::ranger(y ~ .,
                         data = as.data.frame(cbind(X_train, y = Y_train)),
                         importance = "impurity")
    
    predictions <- predict(rf, data = as.data.frame(X_test))$predictions
    ...
    return(list(rf.fit = ss, preds = predictions, true = Y_test))
}
  ...
  return(list(rf.fit = RF.table, mod.tab = model_table, preds = yhat_rf, true = y))
}
...
xgboost_model <- function(X, y, k = 10, n = 500) {
  y <- y[is.finite(y)]  # only finite values
  cl <- sample(intersect(rownames(X), names(y)))  # overlapping rows
  ...
  for (kx in 1:k) {
    ...
    dtrain <- xgboost::xgb.DMatrix(data = as.matrix(X_train), label = y[train])
    dtest <- xgboost::xgb.DMatrix(data = as.matrix(X_test[, colnames(X_train)]))
    ...
    # make predictions
    yhat_xgb[test] <- predict(xgb, newdata = dtest)
    
    # get feature importance
    importance_matrix <- xgboost::xgb.importance(model = xgb)
    ss <- tibble(feature = importance_matrix$Feature,
                 RF.imp = importance_matrix$Gain / sum(importance_matrix$Gain),
                 fold = kx)
  ...
  # table of model level statistics
  mse <- mean((yhat_xgb - y)^2)
  mse.se <- sqrt(var((yhat_xgb - y)^2))/sqrt(length(y))
  r2 <- 1 - (mse/var(y))
  ps <- cor(y, yhat_xgb, use = "pairwise.complete.obs")
  model_table = tibble(MSE = mse,
                       MSE.se = mse.se,
                       R2 = r2,
                       PearsonScore = ps)
  
  return(list(rf.fit = RF.table, mod.tab = model_table, preds = yhat_xgb, true = y))
}
