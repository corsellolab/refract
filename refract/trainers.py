...
from shap import LinearExplainer, TreeExplainer, summary_plot
...
from .utils import (
    ...
    intersect_depmap_ids,
    load_feature_df,
    load_response_df,
    load_selected_features,
    load_split,
)
from .visualize import save_prediction_scatter, save_shap_summary_plot
...
class BaseTrainer(ABC):
    ...
    def train_cross_validation(
        ...
        **kwargs: Additional model-specific parameters
        ...
        for fold in range(n_splits):
            print(f"\n{'='*60}")
            print(
                f"PROCESSING FOLD {fold+1}/{n_splits} for {drug_name} ({response_name})"
            )
            print(f"{'='*60}")
            ...
            print(
                f"Data split: {len(train_ids)} train, {len(val_ids)} validation, {len(test_ids)} test samples"
            )
        ...
        overall_val_metrics = evaluate_predictions(
            np.array(all_val_true), np.array(all_val_pred), "Overall Validation"
        )
        overall_test_metrics = evaluate_predictions(
            np.array(all_test_true), np.array(all_test_pred), "Overall Test"
        )
        ...
        save_prediction_scatter(
            all_test_pred,
            all_test_true,
            os.path.join(self.output_dir, "predictions_scatter.png"),
        )
        ...
        response_df = load_response_df(response_file)
        response_df.to_csv(
            os.path.join(self.output_dir, "response_file.csv"), index=False
        )
    ...
    def save_summary_results(self):
        ...
        results_df.to_csv(
            os.path.join(self.output_dir, "model_performance_summary.csv"), index=False
        )

    def save_all_test_predictions(self, all_test_ids, all_test_true, all_test_pred):
        """Save all test predictions to CSV file."""
        results_df = pd.DataFrame(
            {"id": all_test_ids, "true": all_test_true, "pred": all_test_pred}
        )
        results_df.to_csv(
            os.path.join(self.output_dir, "all_test_predictions.csv"), index=False
        )
    ...
    def print_summary_statistics(self):
        ...
        results_df = pd.DataFrame(
            self.fold_results[:-1]
        )  # Exclude overall results for mean/std calculation

        print("\nPer-fold Summary Statistics:")
        print("\nValidation Metrics:")
        print(
            f"Mean RMSE: {results_df['val_rmse'].mean():.4f} ± {results_df['val_rmse'].std():.4f}"
        )
        print(
            f"Mean R2: {results_df['val_r2'].mean():.4f} ± {results_df['val_r2'].std():.4f}"
        )
        print(
            f"Mean Pearson: {results_df['val_pearson'].mean():.4f} ± {results_df['val_pearson'].std():.4f}"
        )
        print("\nTest Metrics:")
        print(
            f"Mean RMSE: {results_df['test_rmse'].mean():.4f} ± {results_df['test_rmse'].std():.4f}"
        )
        print(
            f"Mean R2: {results_df['test_r2'].mean():.4f} ± {results_df['test_r2'].std():.4f}"
        )
        print(
            f"Mean Pearson: {results_df['test_pearson'].mean():.4f} ± {results_df['test_pearson'].std():.4f}"
        )
    ...
    def save_models(self):
        ...
        for i, model in enumerate(self.models):
            with open(
                os.path.join(self.output_dir, "models", f"model_{i}.pkl"), "wb"
            ) as f:
                pickle.dump(model, f)
...
class XGBoostTrainer(BaseTrainer):
    ...
    def _compute_cv_shap_importance(
        ...
        feature_importance = self._compute_top_features_by_shap(
            combined_shap_values, combined_test_data.columns.tolist()
        )
    ...
    def save_feature_importance(self):
        ...
        feature_importance.to_csv(
            os.path.join(
                self.output_dir, "feature_importance", "feature_importance.csv"
            ),
            index=False,
        )
        shap_values.to_csv(
            os.path.join(self.output_dir, "feature_importance", "shap_values.csv"),
            index=False,
        )
        test_data.to_csv(
            os.path.join(self.output_dir, "feature_importance", "test_data.csv"),
            index=False,
        )
        ...
        save_shap_summary_plot(
            shap_values,
            test_data,
            os.path.join(self.output_dir, "feature_importance", "shap_summary.png"),
        )
    ...
    def train_single_model(self, X_train, y_train, X_val, y_val, **kwargs):
        ...
        **kwargs: Additional parameters (currently unused)
        ...
        for i, param_combination in enumerate(product(*param_values)):
            ...
            if (i + 1) % 10 == 0:
                print(
                    f"Completed {i + 1} parameter combinations, best RMSE so far: {best_score:.4f}"
                )
...
class QuantileRegressionTrainer(BaseTrainer):
    ...
    def train_single_model(self, X_train, y_train, X_val, y_val, **kwargs):
        ...
        **kwargs: Additional parameters (currently unused)
        ...
        print(
            f"Testing {len(self.alphas) * len(self.solvers)} parameter combinations for quantile {self.quantile}..."
        )
        ...
        for alpha in self.alphas:
            for solver in self.solvers:
                try:
                    ...
                    print(
                        f"Alpha: {alpha}, Solver: {solver}, Pinball Loss: {val_loss:.4f}"
                    )
        ...
        if best_model is None:
            raise RuntimeError(
                "No valid model found. All parameter combinations failed."
            )
...
class NeuralNetworkQuantileTrainer(BaseTrainer):
    ...
    def _create_model(
        self, input_size: int, hidden_sizes: List[int], dropout_rate: float
    ):
    ...
    def _train_neural_network(
        self, model, train_loader, val_loader, learning_rate: float
    ):
        ...
        for epoch in range(self.n_epochs):
            ...
            if (epoch + 1) % 10 == 0:
                print(
                    f"  Epoch {epoch+1}/{self.n_epochs} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}"
                )
    ...
    def train_single_model(self, X_train, y_train, X_val, y_val, **kwargs):
        ...
        **kwargs: Additional parameters (currently unused)
        ...
        print(
            f"Training model with architecture: {self.hidden_sizes}, "
    ...
    def load_trainer(cls, output_dir):
        ...
        trainer.device = torch.device(
            trainer_state["device_str"]
            if torch.cuda.is_available() and "cuda" in trainer_state["device_str"]
            else "cpu"
        )
        ...
        if os.path.exists(models_dir):
            ...
            for i in range(trainer_state["num_models"]):
                ...
                model = trainer._create_model(
                    input_size, trainer.hidden_sizes, trainer.dropout_rate
                )
